...
select extensions.is((select count(*) from public.brands), 1::bigint, 'anon reads only active brands');
select extensions.is((select count(*) from public.locations), 1::bigint, 'anon hides inactive locations and locations of inactive brands');
select extensions.is((select count(*) from public.categories), 1::bigint, 'anon hides inactive categories and categories of inactive brands');
select extensions.is((select count(*) from public.products), 2::bigint, 'anon hides inactive products and products with inactive parents');
select extensions.is((select count(*) from public.modifier_groups), 2::bigint, 'anon hides inactive modifier groups and groups of inactive brands');
select extensions.is((select count(*) from public.modifier_options), 2::bigint, 'anon hides inactive options and options with inactive parents');
select extensions.is((select count(*) from public.category_modifier_groups), 1::bigint, 'anon reads only active inherited assignments');
select extensions.is((select count(*) from public.category_modifier_group_exclusions), 1::bigint, 'anon reads only exclusions with visible endpoints');
select extensions.is((select count(*) from public.product_modifier_groups), 1::bigint, 'anon reads only direct assignments with visible endpoints');
select extensions.is((select count(*) from public.modifier_conditions), 1::bigint, 'anon reads only conditions with visible endpoints');
select extensions.is((select count(*) from public.location_products), 1::bigint, 'anon reads only true product availability with visible endpoints');
select extensions.is((select count(*) from public.location_modifier_options), 1::bigint, 'anon reads only true option availability with visible endpoints');
...
select extensions.is((select count(*) from public.brands), 1::bigint, 'authenticated reads the same active brands');
select extensions.is((select count(*) from public.locations), 1::bigint, 'authenticated reads the same active locations');
select extensions.is((select count(*) from public.categories), 1::bigint, 'authenticated reads the same active categories');
select extensions.is((select count(*) from public.products), 2::bigint, 'authenticated reads the same active products');
select extensions.is((select count(*) from public.modifier_groups), 2::bigint, 'authenticated reads the same active modifier groups');
select extensions.is((select count(*) from public.modifier_options), 2::bigint, 'authenticated reads the same active modifier options');
select extensions.is((select count(*) from public.category_modifier_groups), 1::bigint, 'authenticated reads the same inherited assignments');
select extensions.is((select count(*) from public.category_modifier_group_exclusions), 1::bigint, 'authenticated reads the same exclusions');
select extensions.is((select count(*) from public.product_modifier_groups), 1::bigint, 'authenticated reads the same direct assignments');
select extensions.is((select count(*) from public.modifier_conditions), 1::bigint, 'authenticated reads the same modifier conditions');
select extensions.is((select count(*) from public.location_products), 1::bigint, 'authenticated reads the same product availability');
select extensions.is((select count(*) from public.location_modifier_options), 1::bigint, 'authenticated reads the same option availability');
